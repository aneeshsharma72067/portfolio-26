...
### Added
- **Dark Hour — hidden Persona 3 midnight takeover** (`src/hooks/useDarkHour.ts`, `src/components/DarkHour.tsx` + `.css`, wired in `App.tsx` / `Terminal.tsx` / `useConsoleEggs.ts`) — inspired by P3's secret "25th hour". Between **00:00:00 and 00:01:59** local time the site is overtaken: the whole page is washed sickly green (hue-shift + drain filter) and its animations freeze ("time stands still"), any playing audio pauses ("electronics die"), and a full-screen atmosphere layers in — the green moon-clock backdrop (`dark-hour.webp`), a glowing green moon, rising coffin silhouettes, film grain + scanlines, a crimson blood-water vignette, and a fading "The Dark Hour has come. / Memento mori." title card. Fires **once per night** (localStorage guard, survives reload mid-window); dismiss early with click/Esc. Force it any time via the terminal **`darkhour`** command or the **`#darkhour`** hash. Restores the previous theme on exit. Honours `prefers-reduced-motion`.
